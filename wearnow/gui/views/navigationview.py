...
"""
Provide the base classes for wearnow' DataView classes
"""
...
from wearnow.tex.utils.db import navigation_label
from wearnow.tex.constfunc import mod_key
...
class NavigationView(PageView):
    ...
    def goto_active(self, active_handle):
        """
        Callback (and usable function) that selects the active textile
        in the display tree.
        """
    ...
    def selected_handles(self):
        """
        Return the active textile's handle in a list. Used for
    ...
    def bookmark_actions(self):
        ...
        self.book_action.add_actions([
            ('AddBook', 'wearnow-bookmark-new', _('_Add Bookmark'),
             '<PRIMARY>d', None, self.add_bookmark),
            ('EditBook', 'wearnow-bookmark-edit',
    ...
    def navigation_actions(self):
        ...
        self._add_action('HomeTextile', 'go-home', _("_Home"),
                         accel="%sHome" % mod_key(),
                         tip=_("Go to the default Garment"), callback=self.home)

        self.other_action = ActionGroup(name=self.title + '/HomeOther')
        self.other_action.add_actions([
                ('SetActive', 'go-home', _("Set _Home Garment"), None,
                 None, self.set_default_textile),
                ])
    ...
    def set_default_textile(self, obj):
        """
        Set the default textile.
        """
        active = self.uistate.get_active('Textile')
        if active:
            self.dbstate.db.set_default_textile_handle(active)

    def home(self, obj):
        """
        Move to the default textile.
        """
        deftextile = self.dbstate.db.get_default_textile()
        if deftextile:
            self.change_active(deftextile.get_handle())
        else:
            from ..dialog import WarningDialog
            WarningDialog(_("No Home Garment"),
                _("You need to set a 'default garment' to go to. "
                  "Select the Garment View, select the garment you want as "
                  "'Home Garment', then confirm your choice "
                  "via the menu Edit ->Set Home Garment."),
    ...
    def jump(self):
        ...
        if dialog.run() == Gtk.ResponseType.OK:
            gid = text.get_text()
            handle = self.get_handle_from_wearnow_id(gid)
    ...
    def get_handle_from_wearnow_id(self, gid):
        """
        Get an object handle from its wearnow ID.
