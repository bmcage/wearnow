...
import wearnow.gui.widgets.progressdialog as progressdlg
from .lru import LRU
...
class Node(object):
    ...
    handle      A wearnow handle.  Can be None if no wearnow object is
    ...
    def set_handle(self, handle, secondary=False):
        """
        Assign the handle of a wearnow object to this node.
        """
...
class TreeBaseModel(GObject.GObject, Gtk.TreeModel):
    ...
    handle2node A dictionary of wearnow handles.  Each entry is a node object.
    ...
    group_can_have_handle :
                      ...
                      showing subnodes
    has_secondary  :  If True, the model contains two wearnow object types.
    ...
    def __rebuild_search(self, dfilter, skip, items, gen_cursor, add_func):
        """
        Rebuild the data map for a single wearnow object type, where a search 
    ...
    def __rebuild_filter(self, dfilter, skip, items, gen_cursor, data_map, 
                         add_func):
        """
        Rebuild the data map for a single wearnow object type, where a filter 
        is applied.
        """
        pmon = progressdlg.ProgressMonitor(progressdlg.GtkProgressDialog, 
                                            popup_time=2)
    ...
    def add_node(self, parent, child, sortkey, handle, add_parent=True,
        ...
        handle      The wearnow handle of the object corresponding to the
    ...
    def get_iter_from_handle(self, handle):
        """
        Get the iter for a wearnow handle. Should return None if iter not 
        visible
        """
        node = self._get_node(handle)
        if node is None:
    ...
    def get_handle_from_iter(self, iter):
        """
        Get the wearnow handle for an iter.  Return None if the iter does
        not correspond to a wearnow object.
        """
    ...
    def _get_value(self, handle, col, secondary=False, store_cache=True):
        """
        Returns the contents of a given column of a wearnow object
        """
