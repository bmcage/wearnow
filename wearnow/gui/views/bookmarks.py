...
"Handle bookmarks for the wearnow interface."
...
#
#-------------------------------------------------------------------------
WIKI_HELP_PAGE = '%s_-_Navigation' % URL_HOMEPAGE
...
class Bookmarks :
    "Handle the bookmarks interface for WearNow."
    ...
    def connect_signals(self):
        """
        Connect the textile-delete signal
        """
    ...
    def add(self, textile_handle):
        """Append the textile to the bottom of the bookmarks."""
        if textile_handle not in self.bookmarks.get():
            self.bookmarks.append(textile_handle)
    ...
    def remove_handles(self, handle_list):
        ...
        (removal when textile is deleted or merged away).
        """
    ...
    def edit(self):
        ...
        attaching the textile object to the corresponding row. The currently
        ...
        for handle in self.bookmarks.get():
            ...
            if obj:
                wearnow_id = obj.get_wearnow_id()
                self.namemodel.add([name, wearnow_id, handle])
...
class TextileBookmarks(ListBookmarks) :
    "Handle the bookmarks interface for WEARNOW."
    ...
    def make_label(self, handle):
        return navigation_label(self.dbstate.db, 'Textile', handle)

    def connect_signals(self):
        self.dbstate.db.connect('textile-delete', self.remove_handles)
        
    def get_bookmarks(self):
        return self.dbstate.db.get_bookmarks()
        
class EnsembleBookmarks(ListBookmarks) :
    "Handle the bookmarks interface for WEARNOW."
    ...
    def make_label(self, handle):
        return navigation_label(self.dbstate.db, 'Ensemble', handle)

    def connect_signals(self):
        self.dbstate.db.connect('ensemble-delete', self.remove_handles)

    def get_bookmarks(self):
        return self.dbstate.db.get_ensemble_bookmarks()
