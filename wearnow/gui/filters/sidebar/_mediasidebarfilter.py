...
from wearnow.tex.filters.rules.media import (RegExpIdOf, HasMedia, HasTag, 
                                             MatchesFilter)
...
class MediaSidebarFilter(SidebarFilter):
    ...
    def get_filter(self):
        ...
        empty = not (gid or title or mime or path or regex or tag or gen)
        if empty:
            generic_filter = None
        else:
            ...
            rule = HasMedia([title, mime, path], use_regex=regex)
            generic_filter.add_rule(rule)
