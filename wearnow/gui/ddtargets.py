...
"""
   ...
   and all_text_targets() or  or all_wearnow_targets() and all_wearnow_types().
...
class _DdTargets(object):
    ...
    def __init__(self):
        ...
        self.TEXTILE_LINK        = _DdType(self, 'textile-link')
        self.ENSEMBLE_LINK        = _DdType(self, 'ensemble-link')
        ...
        # wearnow widgets but should not be exported
        # to non wearnow widgets.
        self._all_wearnow_types = [
            self.ATTRIBUTE, 
            self.CHILDREF, 
            self.MEDIAOBJ, 
            self.MEDIAREF, 
    ...
    def is_wearnow_type(self, type_name):
        return type_name in self.all_wearnow_types()

    def all_wearnow_types(self):
        """Return a list of all the type names that are internal to wearnow."""

        return tuple([t.drag_type for t in self._all_wearnow_types])
    ...
    def all_wearnow_targets(self):
        """Return a list off the internal wearnow targets."""

        return tuple([t.target() for t in self._all_wearnow_types])

    def all_targets(self):
        """Return a list of all the known targets."""
        return self.all_wearnow_targets() + self.all_text_targets()

    def all_dtype(self):
        """Return all known Ddtype"""
        return [x for x in self._all_wearnow_types] + \
