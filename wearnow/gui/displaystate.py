...
from .managedwindow import WearNowWindowManager
...
from wearnow.tex.recentfiles import RecentFiles
...
class RecentDocsMenu(object):
    ...
    def build(self):
        ...
        actions = []
        rfiles = wearnow_rf.wearnow_recent_files
...
from .logger._rotatehandler import RotateHandler
...
class DisplayState(Callback):
    ...
    def __init__(self, window, status, uimanager, viewmanager=None):
        ...
        Callback.__init__(self)
        self.status = status
        self.status_id = status.get_context_id('WEARNOW')
        self.progress = status.get_progress_bar()
        self.history_lookup = {}
        self.gwm = WearNowWindowManager(uimanager)
        self.widget = None
        #self.disprel_old = ''
    ...
    def modify_statusbar(self, dbstate, active=None):
        ...
        if nav_type == 'Textile' and active_handle \
