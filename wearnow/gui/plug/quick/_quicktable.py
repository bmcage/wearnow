...
from wearnow.tex.simple import SimpleTable
from wearnow.tex.errors import WindowActiveError
from wearnow.gui.utils import model_to_text, text_to_clipboard
    ...
    def button_press_event(self, treeview, event):
        ...
        if index is not None:
            ...
            if (self._callback_leftclick or 
                ...
                objclass, handle = self._link[index]
                if objclass == 'Textile':
                    menu_item = Gtk.MenuItem(label=_("the object|Make %s active") % glocale.trans_objclass('Textile'))
                    menu_item.connect("activate", 
            ...
            if (self.simpledoc.doc.dbstate.db != 
                ...
                if (objclass == 'Filter' and 
                    handle[0] in ['Textile', 'Ensemble', 'Place', 'Event',
    ...
    def on_table_doubleclick(self, obj):
        ...
        if self._callback_leftdouble:
            ...
            if isinstance(handle, list):
                handle = handle[0]
            if objclass == 'Textile':
                textile = self.access.dbase.get_textile_from_handle(handle)
                if textile:
                    from ...editors import EditTextile
                    try:
                        EditTextile(self.simpledoc.doc.dbstate, 
                                   self.simpledoc.doc.uistate, [], textile)
                        return True # handled event
                    except WindowActiveError:
                        pass
            elif objclass == 'Ensemble':
                ref = self.access.dbase.get_ensemble_from_handle(handle)
                if ref:
                    from ...editors import EditEnsemble
                    try:
                        EditEnsemble(self.simpledoc.doc.dbstate, 
    ...
    def on_table_click(self, obj):
        ...
        if self._callback_leftclick:
            ...
            if objclass == 'Textile':
                ...
                #self.simpledoc.doc.uistate.set_active(handle, 'Textile')
                # So, let's return from this, then change the active textile:
                return GLib.timeout_add(100, self.simpledoc.doc.uistate.set_active, handle, 'Textile')
                return True
        return False # didn't handle event
