...
from wearnow.tex.plug import (CATEGORY_QR_TEXTILE, CATEGORY_QR_ENSEMBLE,
                              CATEGORY_QR_MEDIA, CATEGORY_QR_MISC,
                              CATEGORY_QR_NOTE,)
...
def create_quickreport_menu(category,dbstate,uistate, handle) :
    """ This functions querries the registered quick reports with 
        ...
        It collects the reports of the requested category, which must be one of
                        CATEGORY_QR_TEXTILE, CATEGORY_QR_ENSEMBLE, CATEGORY_QR_MEDIA,
...
def get_quick_report_list(qv_category=None):
    ...
    CATEGORY_QR_TEXTILE, CATEGORY_QR_ENSEMBLE, CATEGORY_QR_MISC, CATEGORY_QR_MEDIA,
     or None for all
    """
    names = []
    pmgr = GuiPluginManager.get_instance()
...
def run_quick_report_by_name_direct(report_name, database, document, handle):
    ...
    if report:
        ...
        from wearnow.tex.simple import make_basic_stylesheet
        ...
        **kwargs are only used for special quick views that allow additional
        ...
        if handle:
            ...
            if isinstance(handle, str): # a handle
                if category == CATEGORY_QR_TEXTILE :
                    obj = dbstate.db.get_textile_from_handle(handle)
                elif category == CATEGORY_QR_ENSEMBLE :
                    obj = dbstate.db.get_ensemble_from_handle(handle)
