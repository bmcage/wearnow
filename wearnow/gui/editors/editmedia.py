...
from wearnow.tex.mime import get_description, get_type
from ..thumbnails import get_thumbnail_image, find_mime_type_pixbuf
from wearnow.tex.utils.file import (media_path_full, find_file, create_checksum)
from .editprimary import EditPrimary
from ..widgets import (MonitoredEntry, PrivacyButton,
                         MonitoredTagList)
from .displaytabs.mediabackreflist import (MediaBackRefList)
...
class EditMedia(EditPrimary):
    ...
    def __init__(self, dbstate, uistate, track, obj, callback=None):
        ...
        EditPrimary.__init__(self, dbstate, uistate, track, obj,
                             ...
                             dbstate.db.get_object_from_wearnow_id, callback)
...
#                                        self.db.readonly)
        ...
        self.gid = MonitoredEntry(self.glade.get_object("gid"),
                                  ...
                                  self.obj.get_wearnow_id, self.db.readonly)
        ...
        with DbTxn('', self.db) as trans:
            if not self.obj.get_handle():
                ...
                if not self.obj.get_wearnow_id():
                    self.obj.set_wearnow_id(self.db.find_next_object_wearnow_id())
...
class DeleteMediaQuery(object):
    ...
    def query_response(self):
        with DbTxn(_("Remove Media Object"), self.db) as trans:
            ...
            (textile_list, ensemble_list) = self.the_lists

            for handle in textile_list:
                textile = self.db.get_textile_from_handle(handle)
                new_list = [photo for photo in textile.get_media_list()
                            if photo.get_reference_handle() != self.media_handle]
                textile.set_media_list(new_list)
                self.db.commit_textile(textile, trans)

            for handle in ensemble_list:
                ensemble = self.db.get_ensemble_from_handle(handle)
                new_list = [photo for photo in ensemble.get_media_list()
                            if photo.get_reference_handle() != self.media_handle]
                ensemble.set_media_list(new_list)
                self.db.commit_ensemble(ensemble, trans)
