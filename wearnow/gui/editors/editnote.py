...
from .displaytabs import WearNowTab, NoteBackRefList
...
class NoteTab(WearNowTab):
    ...
    def __init__(self, dbstate, uistate, track, name, widget):
        ...
        the database, along with other state information. The WearNowTab
        ...
        """
        WearNowTab.__init__(self, dbstate, uistate, track, name)
...
class EditNote(EditPrimary):
    def __init__(self, dbstate, uistate, track, note, callback=None, 
        ...
        EditPrimary.__init__(self, dbstate, uistate, track, note, 
                             ...
                             dbstate.db.get_note_from_wearnow_id, callback)
        ...
        if self.obj.get_handle():
            if self.callertitle :
                title = _('Note: %(id)s - %(context)s') % {
                    'id'      : self.obj.get_wearnow_id(),
                    'context' : self.callertitle
                }
            else :
                title = _('Note: %s') % self.obj.get_wearnow_id()
        else:
            if self.callertitle :
    ...
    def _local_init(self):
        ...
        #recreate start page as WearNowTab
        ...
        with DbTxn('', self.db) as trans:
            if not self.obj.get_handle():
                ...
                if not self.obj.get_wearnow_id():
                    self.obj.set_wearnow_id(self.db.find_next_note_wearnow_id())
...
class DeleteNoteQuery(object):
    ...
    def query_response(self):
        with DbTxn(_("Delete Note (%s)") % self.note.get_wearnow_id(),
                   self.db) as trans:
            self.db.disable_signals()
        
            (textile_list, ) = self.the_lists

            note_handle = self.note.get_handle()

            for handle in textile_list:
                textile = self.db.get_textile_from_handle(handle)
                textile.remove_note(note_handle)
