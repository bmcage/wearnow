...
from .displaytabs import WearNowTab
...
class EditReference(ManagedWindow, DbGUIElement):
    ...
    def check_for_duplicate_id(self, type):
        """
        check to see if the wearnow ID (if any) already exists

        type    : the wearnow primary object type, a string
        returns : True if the wearnow ID already exists, else False

        N.B. the various strings, string variables, and titles existed already
        """
        new_id = self.source.get_wearnow_id()
        if new_id:
            old_primary = self.db.get_from_name_and_wearnow_id(type, new_id)
            if old_primary:
                description = None
                if type == 'Event':
                ...
                if description:
                    msg2 = _("You have attempted to use the existing wearnow "
