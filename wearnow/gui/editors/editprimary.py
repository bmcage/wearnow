...
from wearnow.tex.lib import PrimaryObject
...
class EditPrimary(ManagedWindow, DbGUIElement):
    ...
    def __init__(self, state, uistate, track, obj, get_from_handle, 
                 get_from_wearnow_id, callback=None):
        """
        ...
        self.get_from_wearnow_id = get_from_wearnow_id
    ...
    def _uses_duplicate_id(self):
        """
        Check whether a changed or added WEARNOW ID already exists in the DB.
        
        Return True if a duplicate WEARNOW ID has been detected.
        ...
        if original and original.get_wearnow_id() == self.obj.get_wearnow_id():
            ...
            if self.get_from_wearnow_id(idval):
                return (True, idval)
            return (False, 0)
