...
_icu_err = None
_hdlr = None
# WearNowLocale initialization comes before command-line argument
...
class WearNowLocale(object):
    ...
    identical parameters will return the same WearNowlocale
    object. Construction with different parameters will result in a
    new WearNowLocale instance with the specified parameters, but any
    ...
    :param domain: The name of the applicable translation file. The default is
                   "wearnow", indicating files in LC_MESSAGES named wearnow.mo.
    ...
    def __new__(cls, localedir=None, lang=None, domain=None, languages=None):
        if not WearNowLocale.__first_instance:
            cls.__first_instance = super(WearNowLocale, cls).__new__(cls)
            cls.__first_instance.initialized = False
            return cls.__first_instance

        if not cls.__first_instance.initialized:
            raise RuntimeError("Second WearNowLocale created before first one was initialized")
        ...
        return super(WearNowLocale, cls).__new__(cls)
...
# dependency, so we rely on the available translations list
    ...
    def __init_first_instance(self):
        ...
        WearNowLocale is returned by default.
        """
        global _hdlr
...
#localize their output, e.g. time.strftime(). NB: encoding is a class variable.
        ...
        # our WearNowTranslation class but that isn't used to retrieve
    ...
    def _get_language_string(self, lang_code):
        ...
        try:
            ...
            try:
                ...
                LOG.debug("WearNow has no translation for %s", lang_code)
                lang = None
...
#-------------------------------------------------------------------------
    ...
    def date_displayer(self):
        ...
        from wearnow.tex.datehandler import LANG_TO_DISPLAY as displayers
        _first = self._WearNowLocale__first_instance
    ...
    def date_parser(self):
        ...
        from wearnow.tex.datehandler import LANG_TO_PARSER as parsers
        _first = self._WearNowLocale__first_instance
...
#-------------------------------------------------------------------------
    ...
    def get_localedomain(self):
        """
        Get the LOCALEDOMAIN used for the WearNow application.
    ...
    def get_addon_translator(self, filename, domain="addon",
        ...
        """
        wearnow_translator = self._get_translation()
        ...
        wearnow_translator.add_fallback(addon_translator)
        return wearnow_translator # with a language fallback
    ...
    def get_type(self, name):
        ...
        :rtype: unicode
        """
        from wearnow.tex.lib.wearnowtype import WearNowType
        return WearNowType.xml_str(name)
...
class Lexeme(str):
    r"""
    Created with :meth:`~WearNowTranslations.lexgettext`
    ...
    see :meth:`~WearNowTranslations.lexgettext` docs 
    for more info.
    ...
    WearNow often needs to compose sentences from several blocks of
...
class WearNowTranslations(gettext.GNUTranslations):
    """
...
class WearNowNullTranslations(gettext.NullTranslations):
    """
