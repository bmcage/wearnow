...
class ResourcePath(object):
    ...
    created.  At startup it finds the paths to WearNow's resource files and
    ...
    def __init__(self):
        ...
        if installed:
            test_path = os.path.join("wearnow", "authors.xml")
        else:
            test_path = os.path.join("data", "authors.xml")
        resource_path = None
        tmp_path = get_env_var('WEARNOW_RESOURCES')
        if (tmp_path and os.path.exists(os.path.join(tmp_path, test_path))):
            ...
            # Let's try to run from source without env['WEARNOW_RESOURCES']:
        ...
        if installed:
            ...
            self.data_dir = os.path.join(resource_path, 'wearnow')
            self.image_dir = os.path.join(resource_path, 'wearnow', 'images')
            self.doc_dir = os.path.join(resource_path, 'doc', 'wearnow')
