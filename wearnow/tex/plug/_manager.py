...
"""
The core of the WearNow plugin system. This module provides capability to load
...
class BasePluginManager(object):
        ...
        if pdata.fpath not in sys.path:
            if pdata.mod_name:
                ...
                try:
                    ...
                    if win():
                        # we don't want to load WearNow core plugin like this
                        # only 3rd party plugins
                        if "wearnow"  in pdata.fpath:
                            try:
                                sys.path.insert(0, ".")
                                oldwd = os.getcwd()
    ...
    def register_option(self, option, guioption):
        ...
        that is not native to WearNow but provided by the plugin writer.
