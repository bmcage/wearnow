...
from wearnow.version import VERSION_TUPLE
...
def load_addon_file(path, callback=None):
    ...
    for gpr_file in [name for name in file_obj.getnames() if name.endswith(".gpr.py")]:
        ...
        for results in globals()["register_results"]:
            wearnow_target_version = results.get("wearnow_target_version", None)
            id = results.get("id", None)
            if wearnow_target_version:
                vtup = version_str_to_tup(wearnow_target_version, 2)
                # Is it for the right version of wearnow?
                if vtup == VERSION_TUPLE[0:2]:
                    # If this version is not installed, or > installed, install it
                    good_gpr.add(gpr_file)
                    if callback:
                        callback("   " + (_("'%s' is for this version of wearnow.") % id)  + "\n")
                else:
                    # If the plugin is for another version; inform and do nothing
                    if callback:
                        callback("   " + (_("'%s' is NOT for this version of wearnow.") % id)  + "\n")
                ...
                # another register function doesn't have wearnow_target_version
                if gpr_file in good_gpr:
                    s.remove(gpr_file)
                if callback:
                    callback("   " + (_("Error: missing wearnow_target_version in '%s'...") % gpr_file)  + "\n")
    registered_count = 0
    if len(good_gpr) > 0:
