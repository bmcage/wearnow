...
#
#-------------------------------------------------------------------------
from wearnow.version import VERSION as WEARNOWVERSION, VERSION_TUPLE
...
SIDEBAR     = 11
DATABASE    = 12
PTYPE       = [REPORT , TOOL, IMPORT, EXPORT, DOCGEN, GENERAL,
               VIEW, SIDEBAR, DATABASE]
PTYPE_STR   = {
        REPORT: _('Report') ,
        TOOL: _('Tool'),
        IMPORT: _('Importer'),
...
class PluginData(object):
    ...
    .. attribute:: load_on_reg
       bool value, if True, the plugin is loaded on wearnow startup. Some 
    ...
    .. attribute: icons
       ...
       eg: 
            [('wearnow_myplugin', _('My Plugin')), 
            ('wearnow_myplugin_open', _('Open Plugin')]
       The icon directory must contain the directories scalable, 48x48, 22x22
       and 16x16 with the icons, eg:
            scalable/wearnow_myplugin.svg
            48x48/wearnow_myplugin.png
            22x22/wearnow_myplugin.png
    .. attribute: icondir
    ...
    def __init__(self):
        ...
        self._version = None
        self._wearnow_target_version = None
    ...
    def _set_wearnow_target_version(self, version):
       self._wearnow_target_version = version

    def _get_wearnow_target_version(self):
        return self._wearnow_target_version
    ...
    wearnow_target_version = property(_get_wearnow_target_version, 
                                     _set_wearnow_target_version) 
    ...
    def scan_dir(self, dir):
        ...
        for filename in os.listdir(dir):
            ...
            for plugin in self.__plugindata[lenpd:]:
                ...
                if not valid_plugin_version(plugin.wearnow_target_version):
                    print(_('ERROR: Plugin file %(filename)s has a version of '
                            '"%(wearnow_target_version)s" which is invalid for wearnow '
                            '"%(wearnow_version)s".' % 
                            {'filename': os.path.join(dir, plugin.fname),
                             'wearnow_version': WEARNOWVERSION,
                             'wearnow_target_version': plugin.wearnow_target_version,}
                            ))
                    rmlist.append(ind)
                    continue
