...
"""
Package providing filtering framework for WEARNOW.
"""
...
class GenericFilter(object):
    ...
    def get_cursor(self, db):
        return db.get_textile_cursor()

    def make_obj(self):
        return Textile()

    def find_from_handle(self, db, handle):
        return db.get_textile_from_handle(handle)
        ...
        if id_list is None:
            with self.get_cursor(db) as cursor:
                for handle, data in cursor:
                    textile = self.make_obj()
                    textile.unserialize(data)
                    if cb_progress:
                        cb_progress()
                    if task(db, textile) != self.invert:
            ...
            for data in id_list:
                ...
                textile = self.find_from_handle(db, handle)
                if cb_progress:
                    cb_progress()
                if task(db, textile) != self.invert:
        ...
        if id_list is None:
            with self.get_cursor(db) as cursor:
                for handle, data in cursor:
                    ...
                    if cb_progress:
                        cb_progress()
                    val = all(rule.apply(db, textile) for rule in flist)
                    if val != self.invert:
                        final_list.append(handle)
        else:
            for data in id_list:
                ...
                if cb_progress:
                    cb_progress()
                val = all(rule.apply(db, textile) for rule in flist if textile)
                if val != self.invert:
                    final_list.append(data)
        return final_list
    ...
    def xor_test(self, db, textile):
        test = False
        for rule in self.flist:
            test = test ^ rule.apply(db, textile)
        return test

    def one_test(self, db, textile):
        found_one = False
        for rule in self.flist:
            if rule.apply(db, textile):
                if found_one:
                    return False    # There can be only one!
                found_one = True
        return found_one

    def or_test(self, db, textile):
        return any(rule.apply(db, textile) for rule in self.flist) 
...
class GenericEnsembleFilter(GenericFilter):

    def __init__(self, source=None):
        GenericFilter.__init__(self, source)

    def get_cursor(self, db):
        return db.get_ensemble_cursor()
...
def GenericFilterFactory(namespace):
    ...
    elif namespace == 'Ensemble':
        return GenericEnsembleFilter
...
class DeferredFilter(GenericFilter):
    ...
    def __init__(self, filter_name, textile_name):
        GenericFilter.__init__(self, None)
        self.name_pair = [filter_name, textile_name]
