...
from wearnow.tex.const import HOME_DIR, WEARNOW_LOCALE as glocale
...
#
#-------------------------------------------------------------------------
WEARNOW_FILENAME = os.path.join(HOME_DIR,"recent-files-wearnow.xml")
MAX_WEARNOW_ITEMS = 10
...
class RecentItem(object):
    """
    Interface to a single WearNow recent-items item
    """
...
class RecentFiles(object):
    ...
    def __init__(self):
        wearnow_parser = RecentParser()
        self.wearnow_recent_files = wearnow_parser.get()

    def add(self,item2add):
        # First we need to walk the existing items to see 
        # if our item is already there
        for item in self.wearnow_recent_files:
        ...
        self.wearnow_recent_files.insert(0,item2add)
    ...
    def rename_filename(self,filename, new_filename):
        ...
        # if our item is already there
        found = False
        for index in range(len(self.wearnow_recent_files)):
            if self.wearnow_recent_files[index].get_name() == filename:
                # Found it -- break here and change that item
                found = True
                break
        if found:
            self.wearnow_recent_files[index].set_name(new_filename)
    ...
    def remove_filename(self,filename):
        ...
        for index in range(len(self.wearnow_recent_files)):
            if self.wearnow_recent_files[index].get_name() == filename:
                ...
                break
        if found:
            self.wearnow_recent_files.pop(index)
    ...
    def do_save(self):
        """
        Saves the current wearnow RecentFiles collection to the associated file.
        """
        with open(os.path.expanduser(WEARNOW_FILENAME), 'w', encoding='utf8') \
            as xml_file:
            if use_lock:
...
def recent_files(filename, name):
    """
    Add an entry to both GNOME and wearnow recent-items storages.
    """

    the_time = int(time.time())
    wearnow_rf = RecentFiles()
    wearnow_item = RecentItem(
        p=filename,
        n=name,
        t=the_time)
    wearnow_rf.add(wearnow_item)
    wearnow_rf.save()

def remove_filename(filename):
    wearnow_rf = RecentFiles()
    wearnow_rf.remove_filename(filename)
    wearnow_rf.save()

def rename_filename(filename, new_filename):
    wearnow_rf = RecentFiles()
    wearnow_rf.rename_filename(filename, new_filename)
    wearnow_rf.save()

def check_if_recent(filename):
    wearnow_rf = RecentFiles()
    return wearnow_rf.check_if_recent(filename)
