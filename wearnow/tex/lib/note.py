...
class Note(BasicPrimaryObject):
    ...
    def serialize(self):
        ...
        """
        return (self.handle, self.wearnow_id, self.text.serialize(), self.format,
    ...
    def unserialize(self, data):
        ...
        :type: data: tuple
        """
        (self.handle, self.wearnow_id, the_text, self.format,
    ...
    def get_links(self):
        ...
        urls, or can be internal to wearnow objects.

        Return examples::

            [("wearnow", "Person", "handle", "7657626365362536"),
        ...
        for styledtext_tag in self.text.get_tags():
            if int(styledtext_tag.name) == StyledTextTagType.LINK:
                if styledtext_tag.value.startswith("wearnow://"):
                    object_class, prop, value = styledtext_tag.value[9:].split("/", 2)
                    retval.append(("wearnow", object_class, prop, value))
                else:
